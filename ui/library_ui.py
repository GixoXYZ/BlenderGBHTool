...
class VIEW3D_PT_library_ui_main(Panel, GBHBasePanel):
    ...
    def draw(self, context):
        ...
        col = box.column()
        header = col.box()
        self._lib_settings_header(context, header)

        if gbh_lib.lib_settings_switch:
            self._lib_settings_menu(context, header)

        self._lib_categories_and_search(col)
        ...
        if gbh_lib.asset_category == "NODES":
            self._lib_nodes_grid(context, layout)
        elif gbh_lib.asset_category == "MATERIALS":
            self._lib_materials_grid(context, layout)

    def _lib_settings_header(self, context, layout):
        pref = context.preferences.addons[GBH_PACKAGE].preferences
        row = layout.row()
        view = row.row()
        wm = context.window_manager
    ...
    def _lib_settings_menu(self, context, layout):
        pref = context.preferences.addons[GBH_PACKAGE].preferences
        col = layout.column()
        col.prop(
            pref,
    ...
    def _lib_categories_and_search(self, layout):
    ...
    def _lib_materials_grid(self, context, layout):
        pref = context.preferences.addons[GBH_PACKAGE].preferences
        wm = context.window_manager
        gbh_lib = wm.gbh_lib
    ...
    def _lib_nodes_grid(self, context, layout):
        pref = context.preferences.addons[GBH_PACKAGE].preferences
        wm = context.window_manager
        gbh_lib = wm.gbh_lib
